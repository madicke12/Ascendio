...
try{
    ...
    if(!currentAccoutn) throw new Error('No user found');

    const currentUsser = await storage.listDocuments(appwriteconfig.databaseId, appwriteconfig.userCollectionId
        [Query.equal('accountId', currentAccoutn.$id)]
    )
    if(!currentUsser) throw new Error('No user found');
    return currentUsser;
}
catch(error){
    console.log(error);
